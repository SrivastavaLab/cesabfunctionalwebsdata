...
ffs <- formulae(~ fpom_g, 
                linear = ~ fpom_ml,
                quadratic = ~ I(fpom_ml^2) + fpom_ml)

# fit all models with the test data:
observed_data <- fpom_fg_data %>% 
  select(fpom_ml, fpom_g)

model_list <- observed_data %>% 
  fit_with(lm, ffs)
...
new_vals <- detritus_wider_FG_g %>% 
  ...
  filter(has_fpom_ml) %>% 
  unnest(data) %>% 
  select(dataset_name, bromeliad_id, fpom_ml)

model_predictions <- model_list %>% 
  map_df(add_predictions, data = new_vals, .id = "model_type")
```
...
fg_mod_boot <- fpom_fg_data %>% 
  select(fpom_ml, fpom_g) %>% 
  bootstrap(500) %>% 
  mutate(mod = map(strap, ~ .x %>% fit_with(lm, ffs)),
         f = map(mod, ~ data_frame(model_type = names(.), model_obj = .))) %>% 
  unnest(f)
...
fg_mod_lines <- fg_mod_boot %>% 
  mutate(model_preds = map(model_obj, ~ add_predictions(model = .x, data = xs, var = "boot"))) %>% 
  unnest(model_preds)

## turn bootstrap replicates into a 95% confidence interval, derived from *Data*
fg_ribbon_interval <- fg_mod_lines %>% 
  nest(-model_type, -fpom_ml) %>% 
...
fg_predictions <- model_predictions %>% 
  select(model_type, fpom_ml, pred) %>% 
  mutate(ymin = NA_real_, ymax = NA_real_)

dummy_dots <- fpom_fg_data %>% 
  select(fpom_ml, fpom_g) %>% 
  {data_frame(model_type = c("linear", "quadratic"), data = list(., .))} %>% 
  unnest(data) %>% 
  mutate(ymin = NA_real_, ymax = NA_real_)

model_line <- model_list %>% 
  map_df(~ add_predictions(data = xs, model = .x), .id = "model_type") %>% 
  mutate(ymin = NA_real_, ymax = NA_real_)

fg_ribbon_interval %>% 
  ggplot(aes(x = fpom_ml, ymin = ymin, ymax = ymax))  +
  facet_wrap(~model_type) + 
  geom_ribbon(fill = "lightgrey") +
  geom_point(aes(y = pred), colour = "red", data = fg_predictions) +
  geom_line(aes(y = pred), data = model_line) +
  geom_point(aes(y = fpom_g), data = dummy_dots) + 
  theme_bw() +
