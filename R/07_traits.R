## combining trait data -- tachet vs other traitsl
...
traits_all <- read_csv("data-raw/01_traits.csv")


# reshape and combine traits ----------------------------------------------
...
## them together.
traits_all_combined <- traits_all %>%
...
traits_all_combined %>%
  ...
  assertr::verify(nrow(.) == nrow(traits_all)) %>%
  write_csv("data-raw/07_traits.csv")
