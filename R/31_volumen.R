...
summ_broms<-read.csv("data-raw/02_broms.csv")
...
summ_broms <- summ_broms%>%
  mutate(max_water= ifelse(visit_id == 286, volu(diameter), max_water))
...
summ_broms <- summ_broms%>%
  mutate(max_water= ifelse(visit_id == 301, volu(diameter,num_leaf), max_water))

#FrenchGuianaAechmea2008,Kaw, Aechmea mertensii,Vmax= exp(0.0292 +0.339*log(fg$diameter) + 1.3563*log(fg$num_leaf))
volu<- function(a,b){exp(0.0292 +0.339*log(a) + 1.3563*log(b))}
summ_broms <- summ_broms%>%
  mutate(max_water= ifelse(visit_id == 296, volu(diameter,num_leaf), max_water))
...
summ_broms <- summ_broms%>%
  mutate(max_water= ifelse(visit_id == 336|visit_id==331, volu(diameter,num_leaf), max_water))
...
summ_broms <- summ_broms%>%
  mutate(max_water= ifelse(visit_id == 106|visit_id==111|visit_id==91|visit_id==96, vol(num_leaf,actual_water), max_water))
...
summ_broms <- summ_broms%>%
  mutate(max_water= ifelse(visit_id%in%c(131,146,151,156,161,166,171,176,181,351,356,361,366), vol(num_leaf), max_water))
...
summ_broms <- summ_broms%>%
  mutate(max_water= ifelse(visit_id==371, vol(num_leaf,leaf_width), max_water))
...
summ_broms <- summ_broms%>%
  mutate(max_water= ifelse(visit_id%in%c(376,391,396,401,406,411,416,421,426,431,436,441,446), vol(actual_water), max_water))
