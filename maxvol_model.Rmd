...
mertensii<-read.csv("data-intermediate/size_mertensii.csv",header=TRUE, sep=";")
combined_data <- read.csv("data-intermediate/size_all_data.csv")

model_mertensii <- lm(log(Vmax) ~ log(NL) + log(Diam1), data = mertensii)

tidy(model_mertensii)

combined_data_mert <- combined_data %>% 
  filter(filename == "mertensii")

model_combined_mertensii <- lm(log(Vmax) ~ log(NL) + log(Diam1), data = combined_data_mert)

all.equal(combined_data_mert$NL, mertensii$NL)
all.equal(combined_data_mert$Diam1, mertensii$Diam1)
combined_data_mert$Diam1; mertensii$Diam1
...
```{r, eval=FALSE}
summary(lm(log(Vmax) ~ log(Diam1) + log(NL), data = mertensii))
...
vriesea<-read.csv("../data-intermediate/size_vriesea.csv",header=TRUE, sep=";")
vriesea_prod<-read.csv("../data-intermediate/size_vriesea_prod.csv",header=TRUE, sep=";")
##merge datasets
...
##start with 2 datasets
aquilega<-read.csv("data-intermediate/size_aquilega_Biog.csv",header=TRUE, sep=";")
aquilegaKT<-read.csv("data-intermediate/size_aquilegaKT.csv",header=TRUE, sep=";")
##merge datasets
...
guzmania<-read.csv("../data-intermediate/size_Guzmania_PR.csv",header=TRUE, sep=";")
##rename natural log transformed variables
