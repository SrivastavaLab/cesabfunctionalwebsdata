...
## Calculating the volume equations
...
par(mfrow=c(1,1),omi=c(1,1,1,1),mar=c(5,6,6,5)*.5)
plot(exp(mertensii$logNL),exp(mertensii$logVmax),
     ylim=c(0,max(exp(mertensii$logVmax)*1.2)),
     xlab="",ylab="")
# lines(exp(xy),exp(pNL.a6))
mtext("Number of leaves",side=1,line=2.5,cex=1.2)
mtext("Max volume (mL)",side=2,line=2.5,cex=1.2)

plot(exp(mertensii$logDiam1),exp(mertensii$logVmax),ylim=c(0,max(exp(mertensii$logVmax))*1.2),xlab="",ylab="")
...
```{r}
##merge datasets
aquilega_biog$dataset<-1 #datafrom biogeogr expt
aquilegaKT$dataset<-2 #data KT algae expt
aquilega_all<-rbind(aquilega_biog,aquilegaKT)
