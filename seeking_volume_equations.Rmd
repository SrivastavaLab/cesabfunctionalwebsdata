...
We begin by calculating the model
...
I am not totally clear on this "prior to 2008/post 2008" thing. Was something different between those years? were the relationships done in different sites?? 

The code used by Fabiola and Ingacio was:
```r
volFrench2007<- function(a){exp(0.611+1.09*log(a))}
summ_broms <- summ_broms %>%
  mutate(max_water= ifelse(visit_id == 286, volFrench2007(diameter), NA))
```
This matches the table of equations!  

Can we recreate this from data?
```{r}
mertensii_model_supp_dat <- Supp_data %>% 
  filter(species == "Aechmaea_mertensii") %>% 
  lm(log(max_water) ~ log(diameter), data  = .)

mertensii_model_supp_dat %>% 
  tidy %>% 
  kable
```

there they are! the same coeffiecients as were previously found.

## _Aechmea mertensii_ after 2008

This is different from the previous one in that it also includes the number of leaves:
...
```
(Why the quadratic term in the parentheses?)

OK, so maybe it is the same data but with a different model? 
```{r}
mertensii_model_supp_dat_post_08 <- Supp_data %>% 
  filter(species == "Aechmaea_mertensii") %>% 
  lm(log(max_water) ~ log(diameter) + log(num_leaf), data  = .)

mertensii_model_supp_dat_post_08 %>% 
  tidy %>% 
  kable
```

Yep! This matches. So, are two different regressions being used here because different variables were measured in different places in the experiment? i.e. do sites before 2008 lack number of leaves, while sites after 2008 possess it? 

indeed, a glance at [the French Guiana Bromeliad data](http://aammd.info/bwg-data/check_frenchguiana.html) shows that leaves were missing in Petit Saut 2007

#### Comparing the 2007 and 2008 models

One has more parameters and few sample sizes, so I expect it would be a crappier model:

```{r}
list(after_2008 = mertensii_model_supp_dat_post_08, before_2008 = mertensii_model_supp_dat) %>% 
  map_df(glance, .id = 'model category') %>% 
  kable
```


### _Vriesea_ 2007, from actual water
...
Some regressions seem to come from the BWG data itself. Where is that data? Can we recreate those relationships too? 

### Tillandsia guatemalensis/turneri/Complanta from Honduras and Colombia

```{r}
BWG_tillandsia <- bromeliad_detritus %>% 
  filter(species %>% str_detect("Tillandsia"))


BWG_tillandsia %>% 
  left_join(dats_date %>% dplyr::select(dataset_id, country)) %>% 
  dplyr::select(species, dataset_name, country) %>% 
  distinct %>% kable
```

OK there are more _Tillandsia_ in the database than just those in Colombia and Honduras! Let instead filter by country:

```{r}
dats_date %>% select(dataset_id, country) %>% filter(country %in% c("Colombia", "Honduras")) %>% 
  left_join(BWG_tillandsia) %>% 
  vis_miss(cluster=TRUE)
```

Well, the regressions were certainly not made from here! There is no observation of `max_water` in Colombia at all. 

On closer inspection of the original script, it turns out that the regressions for _Tillandsia_ were done with _Guzmania_:
...
Where were the Guzmania measured? 
```{r}
...
```{r fig.cap="data present for all the _Guzmania_ spp."}
library(ggmissing)
ggplot(only_guzmania, aes(x = num_leaf, y = max_water, size = actual_water)) + 
  geom_missing_point()
```

```{r fig.cap = "missing values for Guzmania data"}
# shape for species
only_guzmania %>% vis_miss(cluster = TRUE)
```
...
ggplot(only_guzmania_complete, 
       aes(x = actual_water,
           y = max_water,
           fill = num_leaf)) + 
  geom_point(aes(size = num_leaf), pch = 21) + theme_bw() + 
...
Unsurprisingly, the data show a triangular relationship -- the actual volume can never be greater than the maximum volume. I feel like that's a **Problem**. 
...
This resembles the equation from the table above, but this is a problem because:

1. it is calculated on _Guzmania_.. can it really be used to predict _Tillandsia_?
2. I think that using actual volume to predict maximum is a bad idea, because it predicts the minimum, not the mean, of maximum volume. 
...
#### Comparing NLS to lm model fits.
```{r}
mertensii_nls_mod <- Supp_data %>% 
  filter(species == "Aechmaea_mertensii") %>% 
  nls(max_water ~ exp(a + b * log(diameter)), start = list(a = 0.6, b = 1), data  = .)

pred <- data_frame(diameter = seq_range(Supp_data %>%  
                                          filter(species == "Aechmaea_mertensii") %>%
                                          .[["diameter"]],
                                        n = 30)) %>% 
  add_predictions(mertensii_nls_mod, var = "nls_fit") %>% 
  add_predictions(mertensii_model_supp_dat, var = "lm_fit") %>% 
  mutate(lm_fit = exp(lm_fit))

Supp_data %>% 
  filter(species == "Aechmaea_mertensii") %>% 
  ggplot(aes(x = diameter, y = max_water)) + geom_point() + 
  geom_line(aes(y = nls_fit), colour = "forestgreen", data = pred) +
  geom_line(aes(y = lm_fit), colour = "darkorange", data = pred) + 
  scale_x_log10() + 
  scale_y_log10()

mertensii_nls_mod %>% tidy
```
